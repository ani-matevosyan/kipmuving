<section class="widget select-box">
  ...
  <div class="text-field">
    <select class="form-control" onchange="window.location = '{{ URL::to('activities' ) }}/' + this.value;">
      @foreach ($activities as $a)
        <option value="{{ $a->id }}" @if($a->id == $activity->id)selected @endif>{{ $a->name }}</option>
      @endforeach
    </select>
  </div>
</section>
<section class="widget the-day">
  ...
  <form action="#" class="raised-form-x" style="max-width:150px;">
    <div class="text-field has-ico calender"><input id="reserve-date-sd" type="text" data-datepicker='{"firstDay": 1, "minDate": 1, "dateFormat": "dd/mm/yy" }' placeholder="Fecha" class="form-control" value="{{ $dt }}"></input></div>
  </form>
</section>
...
<section class="important-block">
  ...
  <div class="img-tour">
    ...
    <div class="row">
      <div class="col-sm-12">
        <a href="/{{ $gallery_images[0] }}" rel="prettyPhoto[gallery1]"><img src="/{{ $gallery_images[0] }}" /></a>
      </div>
    </div>
    @endif
    <div class="row">
      ...
      <div class="col-sm-6">
        <a href="/{{ $gallery_images[$i] }}" rel="prettyPhoto[gallery1]"><img src="/{{ $gallery_images[$i] }}" /></a>
      </div>
      @endfor
    </div>
  </div>
